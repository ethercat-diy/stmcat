...
#define hspi9252 hspi2
//extern SPI_HandleTypeDef  hspi3;	//CPOL=0 CPHA=0, copied from Easycat code
extern SPI_HandleTypeDef  hspi2;	//Compatible for Arduino 11,13,12,and PIN9 for CS



void spi_select_lan9252()
{
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_7, GPIO_PIN_RESET);
}

void spi_unselect_lan9252()
{
...
static void lan9252_write_32 (uint16_t address, uint32_t val)
{
    ...
    spi_select_lan9252 ();
    /* Write data */
    HAL_SPI_Transmit(&hspi9252, data, sizeof(data), 2);
    //write (lan9252, data, sizeof(data));
    /* Un-select device. */
    spi_unselect_lan9252 ();
}
...
static uint32_t lan9252_read_32 (uint32_t address)
{
   ...
   HAL_SPI_Transmit(&hspi9252, data, sizeof(data), 2);
   //write (lan9252, data, sizeof(data));
   HAL_SPI_TransmitReceive(&hspi9252, result, result, sizeof(result),2);
...
//      read (lan9252, (temp_buf + byte_offset), sizeof(uint32_t));
      HAL_SPI_TransmitReceive(&hspi9252, (temp_buf + byte_offset), (temp_buf + byte_offset), sizeof(uint32_t),2);
...
//      write (lan9252, (void *)&value, sizeof(value));
      HAL_SPI_Transmit(&hspi9252, (uint8_t*)&value, sizeof(value), 2);
